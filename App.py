...
# Database Veriable set constant 
DATABASE = 'Movie_Data.db'


# Needed secret key for session to work 
app.config['SECRET_KEY'] = '9b25226542c805a3c8a1b625cc5c2cda3bc35e23'
...
@app.route("/")
def Test_Route():
    sql = '''SELECT show_id, title, Year, Rating_image, Poster_image, 
        Rating
        FROM Shows_display
...
def Show_Page(Show_id):
    sql = '''SELECT Shows_display.Show_id, title, 
        ...
        actors, Date_added, Year, Rating_image, Duration, Description, 
        Poster_image, Director.Director_id, Country.Country_id, 
        Genre.Genre_id, Rating
        FROM Shows_display
        JOIN Age_rating ON Shows_Display.Rating_id = Age_Rating.Rating_id
...
def Director_Movies(Director_id):
    sql = '''SELECT Shows_display.Show_id, title, Year, Rating_image, 
        Poster_image, Rating, Director.Directors
        FROM Shows_display
        JOIN Age_rating ON Shows_Display.Rating_id = Age_Rating.Rating_id
...
if __name__ == "__main__":
    # Debug set to False so error 500 page will show for end user
    app.run(debug = False)